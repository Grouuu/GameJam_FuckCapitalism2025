...
namespace Grouuu.UI
{
	...
	public class EventPanelUI : MonoBehaviour
	{
		...
		{
			Sprite sprite = Resources.Load<Sprite>($"{GameController.GameManagers.UiManager.SpritesFolderPath}{FOLDER_SPRITES}{fileName}");
		...
		private void AddResourcesValue (ResultVarChange[] resourceChanges)
		{
			...
			foreach (ResultVarChange change in resourceChanges)
			{
				...
				if (varData != null && varData.varId != GameVarId.None && varData.type == GameVarType.UIVar)
					GameController.GameManagers.UiManager.AddResourceValue(varData, change.currentValue, resourcesParent, Color.black, Color.white);
			}
		}
	}
}
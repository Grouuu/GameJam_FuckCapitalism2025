...
{
	public class CharactersManager : IManager
	{
		private readonly CharactersManagerSettings _settings;
		private CharacterData[] _characters = new CharacterData[0];

		public CharactersManager (CharactersManagerSettings settings)
		{
			_settings = settings;
		}
		...
		public (CharacterData, DialogData) PickDialog (string[] ignoredCharacters)
		{
			...
			List<(CharacterData, DialogData)>[] dialogsByPriority = new List<(CharacterData, DialogData)>[_settings.NumberOfDialogPriorities];
			...
			foreach (CharacterData character in availableCharacters)
			{
				foreach (DialogData dialog in character.characterDialogs)
				{
					...
					if (currentDay != 0)
					{
						...
						{
							bool isResourceLow = GameController.GameManagers.VarsManager.IsVarLow(varId);
		...
		public void UpdateDialogsUsedSaveData ()
		{
			...
			GameController.GameManagers.SaveManager.AddToSaveData(SaveItemKey.DialogsUsed, dialogsUsed);
		}

		public void UpdateCharactersPlayedTodaySaveData (List<string> charactersName)
		{
			GameController.GameManagers.SaveManager.AddToSaveData(SaveItemKey.CharactersPlayedToday, charactersName);
		}

		public void UpdateDialogStartedSaveData (string dialogName)
		{
			GameController.GameManagers.SaveManager.AddToSaveData(SaveItemKey.DialogStarted, dialogName);
		}

		public void UpdateTotalDialogPlayedTodaySaveData (int total)
		{
			GameController.GameManagers.SaveManager.AddToSaveData(SaveItemKey.DialogsPlayedToday, total);
		}

		public void ApplySave ()
		{
			List<string> dialogsUsed = GameController.GameManagers.SaveManager.GetSaveData<List<string>>(SaveItemKey.DialogsUsed);

...
{
	public class VarsManager : IManager
	{
		private readonly VarsManagerSettings _settings;
		private VarData[] _gameVars = new VarData[0];
		private Dictionary<GameVarId, int> _startDayResources = new();

		public VarsManager (VarsManagerSettings settings)
		{
			_settings = settings;
		}
		...
		public void UpdateSaveData ()
		{
			...
			GameController.GameManagers.SaveManager.AddToSaveData(SaveItemKey.VarsValue, varsValue);
			GameController.GameManagers.SaveManager.AddToSaveData(SaveItemKey.StartDayVarsValues, startDayVarsValue);
		}

		public void ApplySave ()
		{
			List<(GameVarId, int, int, int)> varsValue = GameController.GameManagers.SaveManager.GetSaveData<List<(GameVarId, int, int, int)>>(SaveItemKey.VarsValue);
			List<KeyValuePair<GameVarId, int>> startDayVarsValue = GameController.GameManagers.SaveManager.GetSaveData<List<KeyValuePair<GameVarId, int>>>(SaveItemKey.StartDayVarsValues);
		...
		private void UpdateUIValue (VarData varData)
		{
			if (varData.type == GameVarType.UIVar)
			{
				GameController.GameManagers.UiManager.SetResourceValue(varData.varId, varData.currentValue, varData.maxValue);

				if (varData.lowThreshold != null)
				{
					bool isLow = varData.lowThreshold.IsValueOK(varData.currentValue);
					GameController.GameManagers.UiManager.ShowResourceLowWarning(varData.varId, isLow);
				}

				bool isMaxed = varData.currentValue >= varData.maxValue;
				GameController.GameManagers.UiManager.ShowResourceMaxWarning(varData.varId, isMaxed);
			}
		}
